...
for (period_val in periods) {
  for (train_perc in perc_train_list) {
    for (stock_name in stock_names){
      ...
      result_df_raw$Date <- as.Date(result_df_raw$Date)
      filter_end_date <- result_df_raw$Date[nrow(result_df_raw)]
      years_num <- as.numeric(gsub("y", "", period_val))
      
      filter_start_date <- filter_end_date - round(365.25 * years_num)
      ...
      cat("Years: ", period_val, "stock: ", stock_name, "Perc: ", formatted_perc)
      output_filepath <- file.path(results_path, paste0("log_time_series/results_", period_val, formatted_perc, ".csv"))
